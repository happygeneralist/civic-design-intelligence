---
type: user_need
id: UN_005
actor: Parent/carer of a young person with SEND
journey_stage: Pathway_planning
need: A parent/carer of a young person with SEND needs to make sure decisions are based on an accurate view of the young person's abilities, needs and communication, so support is not built on incorrect assumptions.
need_level: journey
parent_needs:
child_needs:
  - "[[UN_006]]"
  - "[[UN_011]]"
related_capabilities:
  - interpretability
  - relational_safety
related_civic_needs:
related_rights:
related_outcomes:
  - accurate support planning
  - reduced harm from misunderstanding
evidence_scope_fit: direct
wording_sensitivity: high
status: draft
analysis_state: evidence_linked
evidence_basis: traceable
evidence_strength: weak
confidence: low
creation_mode: llm_assisted
llm_generated: true
human_reviewed: false
review_status: needs_review
change_level: none
supersedes:
superseded_by:
related_evidence:
  - "[[EVID_004]]"
related_behaviours:
related_pain_points:
related_insights:
importance: high
tags:
  - send
  - pathway-planning
  - parent-carer
  - communication
  - accurate-representation
---

# Make decisions from an accurate view of the young person

## Working formulation

A parent/carer of a young person with SEND needs to make sure decisions are based on an accurate view of the young person's abilities, needs and communication, so support is not built on incorrect assumptions.

## Wording rationale

The wording keeps the parent/carer as the actor while preserving the evidence about inaccurate representation in professional conversations. This is marked high sensitivity because wording can easily imply blame towards either the young person or professionals. The intended meaning is that planning should account for communication context and avoid decisions being built on misleading or partial information.

## Evidence basis

- Supporting evidence: [[EVID_004]]
- Contextual evidence: [[RS_007]]
- Contradictory or limiting evidence:

## Interpretation

The evidence suggests that pathway planning can be undermined when conversations and reports do not reflect how the young person actually functions. The need is for decisions to be grounded in a truthful, contextual understanding of the young person, not a generic or one-off interaction.

## Need hierarchy

- Parent needs:
- Child needs: [[UN_006]], [[UN_011]]
- Related capabilities: interpretability; relational safety
- Related civic needs:
- Related rights:
- Related outcomes: accurate support planning; reduced harm from misunderstanding

## Related problem-space objects

- Behaviours:
- Pain points:
- Insights:
- Value dimensions: dignity; safety; decision quality

## What remains uncertain

- The specific interaction or report type needs checking against the original source.
- Further evidence is needed to understand whether this is a repeated pattern across participants.

## Review notes

- Review date: TBC
- Reviewer: TBC
- Outcome: needs review
- Notes: Review wording carefully because this need involves communication, interpretation and professional judgement.

## Changelog

- 2026-06-12: Created as draft user need from [[EVID_004]]. Status: draft. Analysis state: evidence_linked. Evidence strength: weak. Review status: needs_review.
