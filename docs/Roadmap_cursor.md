...
- use the source capture workflow before decomposing raw sources, public secondary sources, uploaded files or source packages
...
- using source capture before evidence extraction where material is a raw source, public secondary source, uploaded file or source package
...
1. Use the non-linear ingestion, input classification, source capture and slice workflow guidance as the operating model for the next ingestion work.
...
4. For each raw source, public secondary source, uploaded file or source package, decide whether source capture should happen before extraction.
5. For each slice, produce reconciliation notes before finalising the PR.
...
13. Note any object relationships that need more precision than generic `related_*` links, but do not change the schema until real examples show the smallest useful convention.
14. Note any candidate need patterns, solution-vector sensitivity or segmentation dimensions that appear during ingestion, but do not add required fields until examples stabilise.
15. Review future ingestion PRs to see which template, context, secondary-evidence, relationship, need-pattern, persona/journey or validation changes are genuinely needed.
...
- Can we capture and map sources without turning source capture into interpretation?
